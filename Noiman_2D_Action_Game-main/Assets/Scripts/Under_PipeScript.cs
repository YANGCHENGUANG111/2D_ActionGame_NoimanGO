...
public class Under_PipeScript : MonoBehaviour
{
    public float delayBeforeTransition = 0.5f; // パイプに入ってからシーンが切り替わるまでの遅延時間
    public float riseSpeed = 1.0f;//上昇速度
    public float targetDepth = 4.3f;//目標の深さ
    ...
    void Update()
    {
        // プレイヤーがパイプに触れている間に上キーが押された場合
        if (isPlayerInPipe && !isEnteringPipe && (Input.GetAxis("Vertical") > 0 || (Input.GetAxis("Vertical") > 0 && Input.GetButton("Jump"))))
        {
            PipeSource.Play();
            // パイプに入る処理を開始
            isEnteringPipe = true;
            ...
            // Debug.Log("Player is in pipe detection area and Up Arrow is pressed");
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("Player"))
        {
            ...
            isPlayerInPipe = true;
            // Debug.Log("Player is in pipe detection area");
        }
    }
